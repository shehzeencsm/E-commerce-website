...
<body class="bg-gray-200">
  ...
  <div class="max-w-7xl mx-auto p-9 grid grid-cols-1 md:grid-cols-2 gap-11">
    ...
    <!-- LEFT IMAGES -->
    <div>
      <img id="mainImage" src="gb12.jpg" class="w-full rounded-xl shadow-lg h-96 object-cover">

      <div class="flex gap-3 mt-4">
        <img onclick="changeImage(this.src)" src="gb12.jpg" class="w-20 h-20 rounded-md cursor-pointer border">
        <img onclick="changeImage(this.src)" src="gb11.jpg" class="w-20 h-20 rounded-md cursor-pointer border">
        <img onclick="changeImage(this.src)" src="gb10.jpg" class="w-20 h-20 rounded-md cursor-pointer border">
        <img onclick="changeImage(this.src)" src="gb9.jpg" class="w-20 h-20 rounded-md cursor-pointer border">
      </div>
    </div>

    <!-- RIGHT PRODUCT DETAILS -->
    <div>
      <h2 class="text-3xl font-bold">Green tousal Bangles</h2>
      <p class="text-gray-600 mt-2 text-2xl font-bold">Premium | Quality</p>

      <p class="text-2xl font-bold text-[#6e510b] mt-3"> Rs. 1,299</p>
...
<div id="cartPanel"
    ...
    <img id="cartImage" src="a5.jpg" class="w-full h-40 object-cover rounded-lg">

    <h3 id="cartName" class="text-xl font-semibold mt-4">Green tousal Bangles</h3>
    <p class="text-[#6e510b] font-bold">Premium | Quality</p>

    <p class="mt-3 text-gray-600"><b>Quantity:</b> <span id="cartQty">1</span></p>

    <p class="text-gray-700 mt-2"><b>Total:</b> Rs. <span id="cartTotal">Rs.  1,299</span></p>

    <p class="mt-4 text-gray-600 font-semibold">More Images:</p>
    <div class="flex gap-3 mt-2">
        <img src="a3.jpg" class="w-20 h-20 rounded-md border">
        <img src="gb2.jpg" class="w-20 h-20 rounded-md border">
        <img src="gb9.jpg" class="w-20 h-20 rounded-md border">
    </div>
...
<div id="checkoutForm" class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white p-6 rounded-lg w-96">
        ...
        <form action="bcard4.php" id="orderForm" method="POST">
  ...
  <!-- JS -->
  <script>
    const PRODUCT_PRICE =  1299; // product price fixed
    ...
    function updateQty(num) {
      ...
      if (newQty >= 1) {
        ...
        // LIVE UPDATE TOTAL IF CART IS OPEN
...
function openCheckoutForm() {
    // Fill the form automatically from cart
...
document.getElementById('orderForm').addEventListener('submit', function(e){
    ...
    alert('Your order has been placed!\n\nProduct: ' + 
          ...
          '\nTotal: ' + document.getElementById('formTotalPrice').value);
    closeCheckoutForm();
    closeCartPanel();
});
